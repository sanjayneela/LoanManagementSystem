...
def approved_request(request, id):
    ...
    loan_obj.save()
    year = loan_obj.year
    print("Welcome to ",loan_obj.customer.user.email)
    import smtplib
    s = smtplib.SMTP('smtp.gmail.com', 587)
    s.starttls()

    # Authentication
    s.login("sanjay72neela@gmail.com", "yuxw ynth lowa taiq")

    message = "Your Loan has been Approved pls Meet Manager at Branch"

    s.sendmail("sanjay72neela@gmail.com", loan_obj.customer.user.email,message)
    ...
    if CustomerLoan.objects.filter(customer=approved_customer).exists():
        ...
        print("Email Address is ",loan_obj.customer.user.email)
...
def rejected_request(request, id):
    ...
    message = "Your Loan has been Rejected pls Meet Manager at Branch"
